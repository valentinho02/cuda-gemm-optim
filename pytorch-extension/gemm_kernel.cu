    ...
    int M, int N, int K)
{
    ...
    for (int t = 0; t < K; t += TILE_K){
        ...
        As[As_row][As_col] = A[(block_row + As_row) * K + t + As_col];
        As[As_row + 8][As_col] = A[(block_row + As_row + 8) * K + t + As_col];
        As[As_row + 16][As_col] = A[(block_row + As_row + 16) * K + t + As_col];
        As[As_row + 24][As_col] = A[(block_row + As_row + 24) * K + t + As_col];

        Bs[Bs_row][Bs_col] = B[(t + Bs_row) * N + Bs_col + block_col];
        Bs[Bs_row + 2][Bs_col] = B[(t + Bs_row + 2) * N + Bs_col + block_col];
        Bs[Bs_row + 4][Bs_col] = B[(t + Bs_row + 4) * N + Bs_col + block_col];
        Bs[Bs_row + 6][Bs_col] = B[(t + Bs_row + 6) * N + Bs_col + block_col];
        __syncthreads();
...
torch::Tensor gemm_cuda_forward(torch::Tensor A, torch::Tensor B){
    ...
    gemm_register_blocking<<<gridSize, blockSize>>>(A.data_ptr<float>(), B.data_ptr<float>(), C.data_ptr<float>(), M, N, K);
